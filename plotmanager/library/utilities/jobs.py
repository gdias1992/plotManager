...
def load_jobs(config_jobs):
    ...
    for info in config_jobs:
        ...
        job.plots_to_trigger = info.get('plots_to_trigger', None)
        job.stagger_plots_to_trigger = info.get('stagger_plots_to_trigger', 1)

        try:
            if job.plots_to_trigger is None or job.plots_to_trigger == 0:
                job.plots_to_trigger = 1
        except:
            # Don't break things if failed
            job.plots_to_trigger = 1
        ...
        job.optimization_max_threads = info.get('optimization_max_threads', 0)
        job.plot_automatically_optimization = info.get('plot_automatically_optimization', False)

        if not job.plot_automatically_optimization:
            job.optimization_max_threads = 0
        else:
            if job.threads > job.optimization_max_threads:
                job.plot_automatically_optimization = False
                job.optimization_max_threads = 0
...
def monitor_jobs_to_start(jobs, running_work, max_concurrent, max_for_phase_1, next_job_work, chia_location,
    ...
    total_plots_threads = 0
    total_phase_1_count = 0
    total_phase_1_threads = 0
    total_non_phase_1_threads = 0    

    for pid in running_work.keys():
        if running_work[pid].current_phase > 1:
            total_plots_threads += 1
            total_non_phase_1_threads += 1
            continue
        else:
            total_phase_1_count += 1
            total_phase_1_threads += running_work[pid].threads
            total_plots_threads += running_work[pid].threads

    for i, job in enumerate(jobs):
        for x in range(job.plots_to_trigger):
            ...
            threads = job.threads
            if job.plot_automatically_optimization:
                plots_to_start = job.max_for_phase_1 - total_phase_1_count
                if total_non_phase_1_threads == 0 and plots_to_start > 0:
                    if plots_to_start > 1:
                        threads = math.floor(job.optimization_max_threads / job.max_for_phase_1)                
                    else:
                        threads = job.optimization_max_threads - total_plots_threads
                        job.plot_automatically_optimization = False
            ...
            if work is None:
                continue
            total_phase_1_count += 1
            total_plots_threads += work.threads
            
            next_log_check = datetime.now()
            running_work[work.pid] = work
            
            if (x+1) < job.plots_to_trigger:
                time.sleep(1)

    return jobs, running_work, next_job_work, next_log_check


def start_work(job, chia_location, log_directory, drives_free_space, threads):
    ...
    logging.info(f'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> OPTIMIZATION: {job.plot_automatically_optimization}')
